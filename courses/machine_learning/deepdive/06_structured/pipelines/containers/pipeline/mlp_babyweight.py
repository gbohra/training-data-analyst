...
def preprocess_train_and_deploy(
    ...
    bucket='ma-poc-automation_cloudbuild-1',
    start_year='2000'
):
    ...
    preprocess = dsl.ContainerOp(
          ...
          image='gcr.io/ma-poc-automation/babyweight-pipeline-bqtocsv:latest',
          arguments=[
            '--project', project,
            '--mode', 'cloud',
            '--bucket', bucket,
    ...
    hparam_train = dsl.ContainerOp(
        name='hypertrain',
        # image needs to be a compile-time string
        image='gcr.io/ma-poc-automation/babyweight-pipeline-hypertrain:latest',
        arguments=[
    ...
    deploy_app = dsl.ContainerOp(
          name='deployapp',
          # image needs to be a compile-time string
          image='gcr.io/ma-poc-automation/babyweight-pipeline-deployapp:latest',
          arguments=[
...
def train_and_deploy(
    ...
    hparam_train = ObjectDict({
      'outputs': {
        'jobname': os.environ.get('HPARAM_JOB', 'babyweight_200422_180634')
      }
    })
...
def train_and_deploy_helper(preprocess, hparam_train):
    ...
    train_tuned = dsl.ContainerOp(
      name='traintuned',
      # image needs to be a compile-time string
      image='gcr.io/ma-poc-automation/babyweight-pipeline-traintuned:latest',
      arguments=[
    ...
    deploy_cmle = dsl.ContainerOp(
      name='deploycmle',
      # image needs to be a compile-time string
      image='gcr.io/ma-poc-automation/babyweight-pipeline-deploycmle:latest',
      arguments=[
